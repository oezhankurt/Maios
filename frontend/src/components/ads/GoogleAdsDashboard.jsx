export default function GoogleAdsDashboard({ campaigns, budget }) {
    ...
    <div>
      ...
      {budget && (
        <div className="card" style={{ marginBottom: 20 }}>
          <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 16 }}>Budget-Verteilung</div>
          <div style={{ display: 'grid', gap: 12 }}>
            {budget.campaigns.map((camp, idx) => (
              <div key={idx}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6, fontSize: 13 }}>
                  <span>{camp.name}</span>
                  <span style={{ fontWeight: 600 }}>€{camp.budget} ({camp.allocation})</span>
                </div>
                <div style={{ height: 8, backgroundColor: 'var(--bg)', borderRadius: 4, overflow: 'hidden' }}>
                  <div
                    style={{
                      height: '100%',
      ...
      <div className="card">
        <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 16 }}>Top Kampagnen</div>
            ...
            <tbody>
              {campaigns.slice(0, 5).map((camp) => (
                <tr key={camp.id}>
                  ...
                  <td>
                    <span
                      className={`badge badge-${camp.status === 'Aktiv' ? 'success' : camp.status === 'Pausiert' ? 'muted' : 'danger'}`}
                    >
                      {camp.status}
                    </span>
                  </td>
