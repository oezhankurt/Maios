// Extract product identifier from Otto product page
function getProductIdentifier() {
  // Otto uses product ID in URL like /p/1234567890/
  const urlMatch = window.location.pathname.match(/\/p\/(\d+)/);
  if (urlMatch) return urlMatch[1];

  // Try to find SKU from page data
  const skuElement = document.querySelector('[data-sku], [data-product-sku]');
  if (skuElement) return skuElement.getAttribute('data-sku') || skuElement.getAttribute('data-product-sku');

  // Look for EAN
  const eanElement = document.querySelector('[data-ean], [data-product-ean]');
  if (eanElement) return eanElement.getAttribute('data-ean') || eanElement.getAttribute('data-product-ean');
...
function createSidebar() {
  ...
  if (!identifier) {
    console.log('Maios: Product identifier not found on this page');
    return;
  }
  ...
  chrome.runtime.sendMessage(
    { type: 'GET_PRODUCT_DATA', sku: identifier },
    handleProductData
  );
}
